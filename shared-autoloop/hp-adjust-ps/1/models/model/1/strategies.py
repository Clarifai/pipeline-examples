...
def schedule_select(current_hps, task_type, iteration, lr_decay, unfreeze):
    ...
    if task_type not in dispatch:
        raise ValueError(
            f"Unknown task_type '{task_type}' for schedule strategy. "
            f"Allowed values: {list(dispatch.keys())}"
        )
    return dispatch[task_type](current_hps, iteration, lr_decay, unfreeze)
