...
class MMDetectionYoloF(VisualDetectorClass):
    ...
    def train(self,
        ...
        if skip_export:
            ...
            # Upload checkpoint to artifact store for downstream steps
            ...
            config_version = ArtifactVersion().upload(
                file_path=str(self.config_py_path),
                artifact_id=artifact_id,
                user_id=user_id, app_id=app_id,
                visibility="private",
            )
            logging.info(f"Config uploaded as artifact version: {config_version.id}")
