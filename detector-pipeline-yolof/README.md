# YOLOF Object Detection Pipeline (`detector-pipeline-yolof`)

A Clarifai pipeline example that fine-tunes a YOLOF (You Only Look One-level Feature) object detector (MMDetection) on a Clarifai dataset and uploads the trained model to Clarifai.
...
- **Get data**: downloads a Clarifai dataset (you supply `dataset_id` / `dataset_version_id`) and converts it to COCO format.
...
`config.yaml` exposes the parameters users typically override (`dataset_id`, `dataset_version_id`, `concepts`, `image_size`, `keep_aspect_ratio`, `max_aspect_ratio`, `batch_size`, `num_epochs`, `min_samples_per_epoch`, `per_item_lrate`, `pretrained_weights`, `frozen_stages`, `inference_max_batch_size`, `seed`). For full descriptions see `detector-pipeline-yolof-ps/config.yaml` and the `train()` method signature in `detector-pipeline-yolof-ps/1/models/model/1/model.py`.
...
This pipeline requires a Clarifai dataset (it is not pre-wired to a public dataset). Pass `dataset_id` (and optionally `dataset_version_id`, `concepts`) at run time. Override more parameters by repeating `--set key=value`.
